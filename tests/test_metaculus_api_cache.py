...
def test_questions_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(MetaculusForecastSource, "list_questions", fake_api_list_questions)
...
def test_comments_cache(tmp_path, monkeypatch):
    ...
    from cafe.forecast.source_metaculus import MetaculusForecastSource
    cache_file = tmp_path / "metaculus_comments_q1.json"
    with open(cache_file, "w") as f:
        json.dump(SAMPLE_COMMENTS, f)
    monkeypatch.setattr(MetaculusForecastSource, "DATA_DIR", str(tmp_path))
    # Should load from cache
    resp = client.get("/metaculus/questions/q1/comments")
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) == 2
    ...
    class FakeC:
        def __init__(self, d):
            ...
            self.id = d["id"]
            self.text = d["text"]
            self.author = type("A", (), {"username": d["author"]["username"]})() if d.get("author") else None
            self.created_at = datetime.fromisoformat(d["created_at"]) if d.get("created_at") else None
            self.vote_score = d["vote_score"]
    def fake_api_list_comments(self, qid):
        return [FakeC(c) for c in SAMPLE_COMMENTS[::-1]]
    monkeypatch.setattr(MetaculusForecastSource, "list_metaculus_comments_for_question", fake_api_list_comments)
    resp = client.get("/metaculus/questions/q1/comments?force_refresh=true")
    assert resp.status_code == 200
    data = resp.json()
